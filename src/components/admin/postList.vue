...
export default {
  ...
  methods: {
    ...
    reload() {
      this.$confirm('此操作将会从[文章文件目录]中读取markdown文件，并加载到列表，现有文章不受影响，是否确认重载？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'success',
        center: true
        ...
        this.$http.post(this.$constant.baseURL + "/article/reload", {}, true)
          .then((res) => {
            ...
            let intervalId = setInterval(() => {
              this.$http.get(this.$constant.baseURL + "/article/reload/task", {}, true)
                .then((res) => {
                  const taskCount = res.data;
                  if (taskCount === 0) {
                    clearInterval(intervalId);
                    this.$message({
                      message: "已全部重载完成！将为您刷新当前页面！",
                      type: "success"
                    });
                    setTimeout(() => {
                      location.reload();
